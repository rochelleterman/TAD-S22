...
--
**Try it out: tinyurl.com/5n84hsp8**
...
1. construct a new label vector $Z$ where $Z_i = y_i$ if $Z_i = k$ and $Z_i = 0$ otherwise.
...
\\
\text{Specificity} & = & \frac{\text{True Nay}}{\text{False Nay} + \text{False Nay}}  \\
\\
...
- Probability: $P(0\vert \theta)$. .accent[But we don't know] $\theta$ ! 
- Likelihood: $L(\theta \vert O) = P(0\vert \theta)$ 
